...
ggplot(df.airport.month.mean.sd, aes(x=month, y=factor(mean), fill=factor(month))) +
  geom_bar(stat='identity') + theme(legend.position="none") + labs(x = "Months", y="Total Mean Delay",title="Plot 5. Sesonal Trends for Delays") +    guides(fill=FALSE) + scale_x_continuous(breaks=unique(df.airport.month.mean.sd$month)) 
