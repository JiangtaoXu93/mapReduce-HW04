...
public class Log {
	...
	static {
		try {
			errLog = new BufferedWriter(new FileWriter(new File("err.log")));
			outLog = new BufferedWriter(new FileWriter(new File("out.log")));
			String s = "Run started at " + new Date().toString() + "\n";
			errLog.write(s);
			errLog.flush();
			outLog.write(s);
			outLog.flush();
